...
public class PolicyIssueController implements PolicyIssueControllerDocs {

    private final PolicyIssueService policyIssueService;

    @GetMapping
    public ResponseEntity<SuccessResponse<PolicyIssuesDto>> readAll() {
        PolicyIssuesDto data = policyIssueService.readAll();

        SuccessResponse<PolicyIssuesDto> response = new SuccessResponse<>(
                200,
                "악용 이슈 조회 성공",
                data
        );
